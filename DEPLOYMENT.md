...
🔴 **Никогда не удаляйте `/tmp/systemd-private-*` — ни руками, ни скриптом.**
Штатный уборщик их не трогает (правило живёт в `/usr/lib/tmpfiles.d/systemd-tmp.conf`,
две строки выше его дублируют на случай другого дистрибутива). Опасна **ручная**
уборка `/tmp`, к которой рано или поздно приходят на общей машине.

Юнит с `PrivateTmp=yes` получает при старте каталог
`/tmp/systemd-private-<boot>-<юнит>-<rnd>`. Он принадлежит root, лежит месяцами и
выглядит как забытый мусор — типовые критерии ручной чистки («старше N дней»,
«никем не открыт») его не спасают: открытых дескрипторов на сам каталог нет, хотя
сервис жив. После удаления сервис продолжает отвечать (namespace уже смонтирован),
но любая команда **внутри** его namespace падает:
```
...
```
Лечится только перезапуском сервиса, а вскрывается в худший момент — когда конфиг
понадобилось перечитать. Измерено: одна ручная уборка снесла шесть таких каталогов
(`caddy`, `polkit`, `logind`, `resolved`, `timesyncd`, `fwupd`); отказ обнаружился
через двадцать дней, когда потребовалось сменить пароль в дашборде.
```bash
# исключать в любой ручной чистке:
find /tmp -maxdepth 1 -mtime +10 ! -name 'systemd-private-*' ...
# проверка: у живых сервисов с PrivateTmp каталоги обязаны существовать
ls -d /tmp/systemd-private-* 2>/dev/null | wc -l
```
...
**Изоляция общего `/tmp` — `UMASK 027`.** Правами на каталог это не чинится:
`/tmp` обязан быть `1777`, и содержимое закрывает только umask того, кто создал
файл. По умолчанию (`UMASK 022` + `USERGROUPS_ENAB yes`) всё, что сотрудники
кладут в `/tmp`, создаётся мирочитаемым — а туда регулярно попадают дампы,
конфиги и выгрузки с боевыми доступами. Разбор — в
[DOCUMENTATION.md](DOCUMENTATION.md), раздел «Гранула: `UMASK 027`».
```bash
sed -i 's/^UMASK.*/UMASK\t\t027/' /etc/login.defs
grep -E '^(UMASK|USERGROUPS_ENAB)' /etc/login.defs   # ждём UMASK 027 + USERGROUPS_ENAB yes
```
**Приёмка — обязательно в НОВОЙ сессии** (umask выдаётся при входе, текущая
живёт со старым):
```bash
sudo -u <сотрудник> bash -lc 'umask; touch /tmp/u-test && ls -l /tmp/u-test'
# ждём 0007 и -rw-rw---- ; затем: rm -f /tmp/u-test
```
⚠️ Грабли, каждая проверена на живой машине:
- **Жёстче — сломается.** `umask 077` обнуляет group-биты, а из них берётся
  маска ACL: настроенный cross-home доступ молча схлопнется в `#effective:---`.
- **Старые файлы не чинятся** — правило работает только на новые. Уже лежащий
  мирочитаемый хлам останется; разбирать отдельно (и лучше руками владельцев).
- **Долгоживущие сессии не подхватывают.** `pam_umask` действует на новых
  PAM-сессиях, а `systemd --user` при `enable-linger` поднят при загрузке вне
  PAM и держит прежний umask до перезапуска сессии или сервера.
- **root тоже попадает под правило** — из-за этого `npm i -g` раскладывает
  пакет как `drwxr-x---` и у сотрудников он не запускается (см. §8).
...
отдаётся обезличенным.

**Логины дашборда.** Новый заводится `codex-usage-passwd <логин> --create`, смена
пароля — та же команда без флага; пароль можно передать через stdin
(`codex-usage-passwd ivan --create --stdin`), чтобы он не светился в `ps aux`.
Логин обязан быть **существующей системной учёткой**: дашборд сопоставляет его
с `/home/<логин>` и с cgroup-слайсом, поэтому `--create` откажет на несуществующей
(сначала §5). Перегенерировать Caddyfile после этого не нужно — маршруты
обезличены (`{http.auth.user.id}`), а генератор сохраняет блок `basic_auth` как
есть. Персональная страница «Мой расход» появится после ближайшей регенерации
(ежечасный крон) или сразу по `codex-usage-cron.sh`.
...
Если развёрнут дашборд (§4.5), заодно заведите ему веб-логин:
`codex-usage-passwd ivan --create`.
...
- отпечаток сервера `known_host.txt` — `ssh-keyscan -t ed25519 <SERVER_IP> > known_host.txt`
  (снимать из доверенной сети). Скрипт установки внесёт его в `~/.ssh/known_hosts`
  и подключится с `StrictHostKeyChecking=yes`; без файла первое подключение
  принимается «на доверии» (`accept-new`).

Генерация папок из шаблонов — см. `setup-new-pc/_templates/` (плейсхолдеры
`__USER__`, `__KEYNAME__`, `__DISP__`).

> 🔴 **Комплект — это копия ключа.** Скажите сотруднику удалить папку после
> установки (скрипт и README напоминают сами): ключ к тому моменту уже в
> `~/.ssh`, а комплект продолжает жить во Входящих, в Загрузках и на флешке.

> ⚠️ **Критичные нюансы кодировки клиентских файлов** (шаблоны уже такие —
> проверяйте после любой правки, редакторы легко сбрасывают BOM):
> ```bash
> head -c3 setup-windows.ps1 | od -An -tx1     # ждём ef bb bf
> file setup-mac.sh УСТАНОВКА.command          # без BOM, LF
> ```
> - **`.ps1`** — UTF-8 **с BOM**. Без него PowerShell 5.1 читает файл в ANSI, и
>   кириллица разваливает разбор: байт `0x93` (внутри буквы «Г») читается как
>   открывающая кавычка, скрипт падает с `Missing closing '}'` ещё до первой
>   строки работы. Синтаксис при этом **корректен** — ошибка чисто в кодировке.
