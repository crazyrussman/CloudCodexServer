...
# Уборка сирот. Ловушки ниже снимают снимок сами, но их не бывает при SIGKILL,
# падении питания и OOM — а каждый забытый снимок это ПОЛНАЯ копия Caddyfile:
# bcrypt-хеши всех логинов и X_AUTH_TOKEN открытым текстом. Прав они не теряют
# (cp -a сохраняет 640 root:caddy), поэтому это гигиена, а не дыра, — но копии
# секретов не должны лежать бессрочно и никем не считаться.
# Порог 60 минут, чтобы не тронуть снимок параллельно идущего запуска.
# Шаблон БЕЗ точки перед звёздочкой: он обязан ловить и `Caddyfile.bak-passwd`
# без суффикса — так снимок назывался в промежуточной версии этого скрипта,
# и именно её сироты переживут любую метлу, написанную под текущий формат имени.
find "$(dirname "$CF")" -maxdepth 1 -name "$(basename "$CF").bak-passwd*" \
...
# Пока копия не снята, откатывать нечем — но пустышку от mktemp за собой убрать
# надо: без этой строки падение `cp -a` (например, на переполненном диске)
# оставляло бы в /etc/caddy файл нулевой длины.
trap 'rm -f "$BAK"' ERR INT TERM HUP
cp -a "$CF" "$BAK"
# `cp -a` переносит и метки времени, поэтому снимок рождается ровно таким же
# «старым», как сам Caddyfile (а он лежит без правок неделями). Без этой строки
# порог -mmin выше не защищает ничего: уборка соседнего запуска снесла бы снимок
# живой правки через секунду после его создания, и откат стал бы пустышкой.
touch "$BAK"
trap restore ERR
