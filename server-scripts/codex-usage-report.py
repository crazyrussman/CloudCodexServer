...
{body}
<p class="foot">Оценка по объёму логов сессий Codex (события за период), НЕ биллинг ChatGPT. Только метаданные сессий — без текста промптов и содержимого файлов; имена участников и проектов видит только администратор.</p>
</div>{JS}</body></html>"""
